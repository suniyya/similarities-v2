...
def get_response_files(subject_name, directory):
    ...
    if len(subject_name) == 0 or subject_name == ' ':
        raise ValueError('Please specify Subject')
    paths = []
    ...
    if len(paths) == 0:
        raise ValueError("No data files found in provided directory")
    return paths
...
if __name__ == '__main__':
    ...
    PARADIGM = None
    while PARADIGM not in ['image_exp', 'working_mem_exp', 'materials_exp']:
        PARADIGM = input("> Name of experiment (choose from [image_exp, working_mem_exp, materials_exp]): ")
    experiment_name = input("> Experiment name: (default= bgca3pt9 )")
    if experiment_name == '' or experiment_name == ' ':
        experiment_name = 'bgca3pt9'

    instructions = input('Was the subject given the standard (similarity) instructions? (y/n)')
    if instructions == 'n':
        raise ValueError('Run analysis.preprocess_dis instead')
    ...
    fieldnames = ['ref'] + ['stim' + str(i) for i in range(1, num_stim_per_trial + 1)] + \
                 ['clicks', 'clicked_stimuli', 'participant', 'session', 'conds file', 'path to stim']
    ...
    for subject in [subject_name]:
        ...
        for file in subject_files:
            ...
            with open(file, 'r', encoding='utf-8-sig') as csv_file:
                reader = csv.DictReader(csv_file, fieldnames=fieldnames)
                reader.__next__()  # to skip header line - BUT ONLY IF YOU PASSED IN FIELDNAMES!!
                for row in reader:
                    if session_num is None:
                        session_num = row['session']
                    # eval is usually not secure but here I created the files parsing
