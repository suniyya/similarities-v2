"""
 ...
 stim1, stim2, ..., stim8, bars clicked, participant, session etc.
 There is no ref in this paradigm. Adjacent pairs of stimuli are compared to each other, and chosen in
 order of similarity, i.e., the pair that seems most like each other are picked first and so on.
...
from analysis.preprocess_uncons_grouping import process_clicks_uncons_gp


def bar_id_to_stimuli(bar_num, max_num=8):
    # bar_num index starts from 1 and at most is equal to the number of stimuli
    if bar_num == max_num:
        return 'stim' + str(max_num), 'stim1'
    else:
        return 'stim' + str(bar_num), 'stim' + str(bar_num + 1)


def process_clicks(subject, paradigm, data_directory, num_stim_per_trial):
    """
    ...
    """
    if paradigm == 'unconstrained_grouping_exp':
        return process_clicks_uncons_gp(subject, data_directory)
        ...
        print('Processing', str(len(subject_files)), 'files for GROUPING exp')
        for file in subject_files:
            ...
            with open(file, 'r', encoding='utf-8-sig') as csv_file:
                ...
                for row in reader:
                    ...
                    if ses_num is None:
                        ses_num = row['session']
                    # for each trial, compile a dictionary of choices and judgments ensure that for a comparison of the
                    # form s1,s2 < s3,s4, s1 and s2 are in alphabetical order and s3 and s4 are in alphabetical order.
                    # This imposes an order on each unique pairwise comparison, so there is no risk for another entry
                    # reading s2-s1 < s3-s4 etc.
                    clicks = ast.literal_eval(row['clicks'])
                    # only use eval if input trusted
                    valid_pairs_after_each_click = eval(row['valid_pairs_after_each_click'])
                    for i in range(len(clicks)):
                        idx_a, idx_b = bar_id_to_stimuli(int(clicks[i][3:]), max_num=num_stim_per_trial)
                        # remove text after underscore
                        ref_pair = sorted([row[idx_a].split('_')[0], row[idx_b].split('_')[0]])
                        comparison_pairs = valid_pairs_after_each_click[i]

                        # for each pair of stimuli (s_m, s_n) that are clicked we have several comparison pairs:
                        #  (s_i, s_j), (s_r, s_t) ... sort the stimuli within each pair => e.g., (s_n, s_m), (s_i, s_j),
                        #  (s_t, s_r). next when recording the counts, write the key based on the alphabetical order
                        # again, meaning if (s_n, s_m) is being compared to (s_i, s_j), the key should be
                        # (s_i, s_j) < (s_n, s_m) if (s_i, s_j) came before (s_n, s_m) in
                        # sorted([(s_n, s_m), (s_i, s_j)]) this will help keep track of repeats of a comparison
                        # regardless of which pair is clicked.
                        for bar_id in comparison_pairs:
                            idx_i, idx_j = bar_id_to_stimuli(int(bar_id[3:]), max_num=num_stim_per_trial)
                            # rm text after underscore
                            comp_pair = sorted([row[idx_i].split('_')[0], row[idx_j].split('_')[0]])
                            pairs = sorted([(comp_pair[0], comp_pair[1]), (ref_pair[0], ref_pair[1])])
                            key = '{},{}<{},{}'.format(pairs[0][0], pairs[0][1], pairs[1][0], pairs[1][1])
                            if key not in repeats:
                                repeats[key] = 1
                                # if ref pair is listed first, then count judgment as choosing ref pair to be more
                                # similar do nothing otherwise, if ref pair is listed second, that implies another pair
                                # was chosen before the ref pair, i.e., the pair that was clicked first. downstream
                                # processing will interpret this as a judgment in the opposite direction
